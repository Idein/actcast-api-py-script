...
def init_interval_settings(api):
  ...
  if 'device_specific_settings_action' in api.setting_json:
    Setting.device_specific_settings_action = DeviceSpecificSettingsAction.value_of(api.setting_json['device_specific_settings_action'])
  if 'enable_rand_maximum_timelimit_minute_for_sb' in api.setting_json:
    Setting.enable_rand_maximum_timelimit_minute_for_sb = api.setting_json['enable_rand_maximum_timelimit_minute_for_sb']
...
def install_act_from_list(api, group_id ,act_id, id_list_name):
    ...
    print(f'Request to install Act: [ {act_id} : {CurrentInfo.act_info.name} ]')
...
if __name__ == '__main__':
  ...
  if len(args) < 4:
    print("usage:")
    print(f"$ python3 {path.basename(__file__)} group_id act_id id_list.txt")
  else:
    group_id  = args[1]
    act_id    = args[2]
    id_list_name   = args[3]
